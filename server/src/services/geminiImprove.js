...
- You MUST carry over the candidate's full contact information exactly as it appears in the original resume — full name, email, phone, location, LinkedIn, and portfolio/GitHub link. Copy these values verbatim; never leave a contact field blank if the original resume contains it.
...
export async function improveResumeForJD(resumeText, jdText, missingKeywords = []) {
  ...
  // with explicit prompt instructions not to. Since email/phone/LinkedIn/
  // GitHub are reliably pattern-matchable from the raw resume text, backfill
  // any field the AI left blank rather than trusting it caught everything.
  const hints = extractContactHints(resumeText)

  return {
    title: typeof parsed.title === 'string' && parsed.title.trim() ? parsed.title.trim() : 'Optimized Resume',
    sections: {
      contact: {
        ...
        linkedin: contact.linkedin || hints.linkedin || '',
        portfolio: contact.portfolio || hints.portfolio || '',
      },
